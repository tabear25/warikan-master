    ...
    items: [
      {
        ...
        answer: (
          <>
            ...
            先に精算した支払いは、精算画面の「先に精算した分」で取り消すと編集・削除できるようになります。
          </>
        ),
        searchText: "鉛筆アイコンから編集 ゴミ箱アイコンから削除 修正 まちがえた 訂正 精算済み 先に精算した支払い",
      },
      {
        id: "split-modes",
      ...
      },
      {
        ...
        answer: (
          <>
            ...
            「銀行振込／PayPay／現金で手渡し／何でもいい」から希望を選べます。
      ...
      },
      {
        ...
        question: "旅行前に、ホテル代や飛行機代だけ先に精算できますか？",
        answer: (
          <>
            できます。精算画面の<strong className="font-semibold text-foreground">「一部だけ先に精算」</strong>から
            先に精算したい支払いを選ぶと、その支払いだけで送金リストが作られます。
            旅程から割り勘に追加した宿泊・移動は「宿泊・移動をまとめて選ぶ」で一度に選べます。
            選んだ支払いは「先に精算済み」になって残りの精算から外れ、旅行中の支払いはこれまでどおり追加できます。
            先に精算した分は精算画面に残るので、コピーしてグループに共有したり、間違えたときに取り消したりできます。
          </>
        ),
        searchText: "部分精算 部分決済 一部だけ 先に精算 先払い 前払い 事前 旅行前 ホテル代 飛行機代 航空券 宿泊費 交通費 分けて精算 途中で精算 精算済み 取り消し",
      },
      {
        id: "share-result",
    ...
    items: [
      {
        ...
        answer: (
          <>
            できます。イベント作成時に種類で「旅行」を選ぶと、イベント画面に「旅程」タブが現れ、
            宿泊・移動・観光などの予定を日付ごとのタイムラインで管理できます。
            予約ページの URL を貼るとタイトルなどが自動で補完され、住所を入れると地図リンクも表示されます。
            ※あとから合言葉チップ横の設定からも変更可能です
          </>
        ),
